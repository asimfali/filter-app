...
**Полностью в проде (2026-08-07):** `filter-app` (кнопки «Графики»/«Генератор документов»
в шапке), `filter-app-graphs` (`portal-core`-миграция включительно) — оба через
`prod.sh`/`deploy_frontend.sh`, `/graphs/` подтверждён рабочим на живом сервере.

Осталось: ничего не блокирует, инфраструктура под следующие вынесенные приложения готова.

## Уроки этого прохода деплоя (пригодятся для следующих приложений)

- `deploy_frontend.sh` падает на `git pull`, если `npm install` на сервере успел переписать
  `package-lock.json` в рабочей копии до коммита (обычное дело — npm часто трогает лок-файл
  даже без реальных изменений зависимостей). Хвост от прошлого/упавшего деплоя ловит
  следующий `git pull`. Фикс: `git checkout -- package-lock.json` перед повторным `git pull`.
  Встречалось трижды за этот заход (`doc-generator`, `filter-app-graphs` дважды) — возможно,
  стоит добавить `git checkout -- package-lock.json` прямо в `deploy_frontend.sh` перед
  `git pull`, чтобы не натыкаться на это каждый раз.
- **Источник правды для nginx** (`drf_catalog_service/systemd/nginx.conf`) и **реально
  применённый конфиг** (`/etc/nginx/sites-available/tmdata` на сервере) — разные файлы;
  первый копируется во второй только через `deploy.sh`'s `setup_services()` (с подстановкой
  `{PROJECT_DIR}`/`{SERVER_IP}`). Правка одного не значит, что применилась в другом — при
  добавлении `/docgen/` источник обновили, а живой файл — нет, из-за чего `/docgen/` полчаса
  проваливался в `location /` портала (пустой экран, а не 404/502 — сбивало с толку). Симптом
  на будущее: если location `^~ /new-path/` "не матчится" и падает в catch-all — первым делом
  сверить `grep -A10 <path> /etc/nginx/sites-available/tmdata` с исходником в репозитории,
  а не гадать про сам сервис за прокси.
