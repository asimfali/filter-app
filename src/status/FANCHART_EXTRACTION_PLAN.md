...
Пункт «Графики» в `Header.jsx` оставлен — теперь это `<a href=GRAPHS_URL>` (env-осознанная,
`:5174` в dev / `/graphs/` в проде) в той же вкладке, не внутренний `page`; внутренний код
страницы удалён из `filter-app` целиком (2026-08-07).

`filter-app-graphs` получил topbar со ссылкой назад в портал (`PORTAL_URL`, аналогично
env-осознанная) — без него не было пути назад из вынесенного приложения.

Заведён `portal-core` (репозиторий `~/develop/portal-core`, remotes `local`+`origin` по
той же схеме) — общий `auth.js`/`permissions.js`/`useAuthGate` для всех будущих вынесенных
приложений (это первое из нескольких, см. обсуждение в чате 2026-08-07). `filter-app-graphs`
уже на нём. `filter-app` сам пока НЕ переведён на `portal-core` — свой `utils/permissions.js`
остаётся источником для портала, синхронизируется с `portal-core` вручную при добавлении
новых `PERM.*` (см. `portal-core/README.md`).

Осталось: ничего не блокирует, инфраструктура под следующие вынесенные приложения готова.
